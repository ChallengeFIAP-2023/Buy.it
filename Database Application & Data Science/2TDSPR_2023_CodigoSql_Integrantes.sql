...
BEGIN
    ...
    IF v_count = 0 THEN
        DBMS_OUTPUT.PUT_LINE('Nenhuma cotaï¿½ï¿½o pendente encontrada para o usuï¿½rio especificado.');
    ELSE
        FOR r IN (
...
EXCEPTION
    WHEN no_data_found THEN
        DBMS_OUTPUT.PUT_LINE('Nenhuma cotaï¿½ï¿½o pendente encontrada para o usuï¿½rio especificado.');
    WHEN value_error THEN
        DBMS_OUTPUT.PUT_LINE('Ocorreu um erro ao recuperar o valor de uma cotaï¿½ï¿½o.');
    WHEN OTHERS THEN
...
EXCEPTION
    WHEN NO_DATA_FOUND THEN
        RAISE_APPLICATION_ERROR(-20003, 'Status "Concluï¿½do" nao encontrado.');
    WHEN OTHERS THEN
...
EXCEPTION
    WHEN no_data_found THEN
        RAISE_APPLICATION_ERROR(-20001, 'Nenhuma cotaï¿½ï¿½o encontrada para o usuï¿½rio especificado.');
    WHEN value_error THEN
        RAISE_APPLICATION_ERROR(-20002, 'O valor total das cotaï¿½ï¿½es ï¿½ negativo.');
    WHEN OTHERS THEN
        RAISE_APPLICATION_ERROR(-20003, 'Erro ao calcular o valor total das cotaï¿½ï¿½es.');
END;
...
EXCEPTION
    WHEN no_data_found THEN
        RAISE_APPLICATION_ERROR(-20001, 'Nenhum usuï¿½rio encontrado com o e-mail especificado.');
    WHEN OTHERS THEN
        RAISE_APPLICATION_ERROR(-20002, 'Erro ao verificar a existï¿½ncia do e-mail do usuï¿½rio.');
END;
