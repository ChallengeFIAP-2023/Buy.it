...
-- 04.05.01 Procedimento listar cotação pendente

CREATE OR REPLACE PROCEDURE listar_cotacoes_pendentes AS
BEGIN
    FOR r IN (
        SELECT c.id_cotacao, c.data_abertura_cotacao, c.quantidade_produto, c.valor_produto, s.nome_status
        FROM cotacao c
        JOIN status s ON c.id_status = s.id_status
        WHERE s.nome_status IN ('Em Andamento', 'Recusado')
        ORDER BY c.data_abertura_cotacao DESC
    ) LOOP
        DBMS_OUTPUT.PUT_LINE('ID da Cotação: ' || r.id_cotacao || 
                             ', Data de Abertura: ' || TO_CHAR(r.data_abertura_cotacao, 'DD/MM/YYYY') || 
...
BEGIN
    listar_cotacoes_pendentes;
END;


-- 04.05.02 Procedimento Concluir cotação
...
-- 04.06.01 Função valor total de cotação
...
-- 04.06.02 Função verificar existencia email
...
-- 04.07.01 Criação de tabelas para o funcionamento do gatilho
...
-- 05.07.01 Gatilho para monitoramento de atualizações
...
-- 04.07.02 Testando o gatilho 
UPDATE cotacao SET valor_produto = 150 WHERE id_cotacao = 20;

SELECT * FROM monitoramento_atualizacoes;

-- Testando a captura de erros do log_erros
...
--------------------------------------------------------------------------------
>>>>>>> bce1f413b465ec11199f022764285d15b74f951b
